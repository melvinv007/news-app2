 ...
 * AI model assignments, prompts, and fallback chain — copied from config/ai.ts.
 */
...
- Output ONLY the slug. Nothing else.`.trim(),
...
Return ONLY valid JSON. No markdown fences. No explanation.`.trim(),
...
Reply with only: YES or NO`.trim(),
...
Return ONLY a JSON array: ["TATAMOTORS.NS", "AAPL"] or []`.trim(),
...
Return ONLY valid JSON. No markdown fences.`.trim(),
};
