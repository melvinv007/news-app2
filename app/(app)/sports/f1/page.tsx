 ...
 * ─────────────────────────────────────────────────────────────────
 * F1 page: calendar, countdown, standings (via articles), and news feed.
 * Category: 'sports-f1'
 *
 * Features:
 *   - F1 calendar with next-race countdown
 *   - F1 standings from DB articles
...
import F1Standings from '@/components/Sports/F1Standings';

const CATEGORY = 'sports-f1';
const PAGE_SIZE = 20;

export default function F1Page(): React.ReactElement {
  ...
  const fetchArticles = useCallback(async (): Promise<void> => {
    ...
    if (error) {
      console.error('[F1] Fetch error:', error.message);
    } else if (data) {
      setArticles(data);
    }
    setLoading(false);
